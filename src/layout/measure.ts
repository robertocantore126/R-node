 ...
 *
 * The one implementation of "cards in a grid" — the gallery and any future
 * grid-shaped body share it, so a change to the pitch never has to find every
 * copy of the three-line arithmetic.
 */
 ...
 * disagreeing about how many pictures fit on a row. Without an explicit
 * `Style.gallery.cols` the grid takes a fixed GALLERY_COLS per row and the box
 * grows to fit it.
 *
 ...
 */
export function galleryExtent(n: MindNode): GalleryExtent | null {
  ...
  if (g.cols && g.cols > 0) {
    ...
    cols = Math.min(count, GALLERY_COLS);
  }
...
export function positionedImageSlots(
  ...
  if (gallery) {
    ...
    const g = n.style.gallery!;
    // gridCells owns the "cards in a wrapping grid" arithmetic; these cells
    // only add the caption band under each picture (which gridCells must not
    // know about — the caption is gallery chrome, not grid geometry).
    const rects = gridCells(g.items.length, gallery.cols, gallery.cellW, gallery.cellH, GALLERY_GAP, gridLeft, gridTop);
    for (let i = 0; i < g.items.length; i++) {
      ...
      cells.push({
        ...
        captionY: r.y + gallery.cellPicH + GALLERY_CAPTION_GAP,
...
function measureTopicUncached(
  ...
  // already bounded — GALLERY_COLS cells by default, or an explicit `cols` —
  // so this widens the box without unbounding it.
